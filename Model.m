...
%   INPUTS:
%   -------
%   species_init    : row vector of 21 initial species populations.
%   timepoints      : column vector of time points returned by integrator.
%   parameters      : row vector of 8 model parameters.
...
if ( isempty(parameters) )
   parameters = [ 0.1, 0.117, 6E-3, 1.34, 3.6E-2, 2.0E-2, 5.8E-2, 4.6E-3 ];
end
% check that parameters has proper dimensions
if (  size(parameters,1) ~= 1  ||  size(parameters,2) ~= 8  )
    fprintf( 1, 'Error: size of parameter argument is invalid! Correct size = [1 8].\n' );
    err = 1;
    return;
end
...
if (  size(species_init,1) ~= 1  ||  size(species_init,2) ~= 21  )
    fprintf( 1, 'Error: size of species_init argument is invalid! Correct size = [1 21].\n' );
    err = 1;
    return;
end
...
param_labels = { 'C1', 'C2', 'C8_A', 'C9', 'C10_A', 'C12_A', 'C12_D', 'C12_E' };
...
observables_out = zeros( length(timepoints), 21 );
...
if ( ~suppress_plot )
    ...
    % define plot labels
    observable_labels = { 'DP3', 'RNAP', 'ssDNA', 'ssPDNA', 'RF1', 'DA', 'DB', 'DH', 'DZ', 'DW', 'EA', 'ELA', 'A', 'D', 'E', 'F', 'RBS2', 'RBS5', 'RBS7', 'RBS9', 'RBS8' };
...
function [ expressions ] = calc_expressions ( parameters )
    ...
    expressions(9) = (0.7*expressions(7));
    expressions(10) = (0.7*expressions(8));
...
function [ ratelaws ] = calc_ratelaws ( species, expressions, observables )
    ...
    ratelaws(6) = expressions(6)*species(13);
    ratelaws(7) = expressions(6)*species(17);
    ratelaws(8) = (0.7*expressions(7))*species(14);
    ratelaws(9) = (0.7*expressions(8))*species(15);
...
function [ Dspecies ] = calc_species_deriv ( time, species, expressions )
    ...
    Dspecies(13) = ratelaws(5) -ratelaws(6);
    Dspecies(14) = ratelaws(6) -ratelaws(8);
    Dspecies(15) = ratelaws(8) -ratelaws(9);
    Dspecies(16) = ratelaws(9);
    Dspecies(17) = ratelaws(5) -ratelaws(7);
