function [err, timepoints, species_out, observables_out] = Swap_4_10( timepoints, species_init, parameters, suppress_plot )
%SWAP_4_10 Integrate reaction network and plot observables.
%   Integrates the reaction network corresponding to the BioNetGen model
%   'Swap_4_10' and then (optionally) plots the observable trajectories,
...
%   'ode15s'. SWAP_4_10 returns an error value, a vector of timepoints,
%   species trajectories, and observable trajectories.
%   
%   [err, timepoints, species_out, observables_out]
%        = Swap_4_10( timepoints, species_init, parameters, suppress_plot )
%
%   INPUTS:
%   -------
...
if ( isempty(parameters) )
   parameters = [ 0.1, 0.125, 1.0, 1.0E3, 1, 9.5454e-16, 1E-3, 1100, 1.0, 6E-3, 1E-2, 6.5E-3, 2E-4, 2.25E-3, 1.34, 0.03643, 0.0351, 0.25, 0.22333, 0.04214, 0.60, 0.02377, 0.02678, 0.00307, 0.00307, 0.00307, 0.00163, 0.00163, 0.00163, 0.00163, 0.00163, 1.60, 10.8, 8.00, 4.60, 20.0, 0.80, 4.40, 1.60, 1.08, 4.00, 0.25, 0.048443, 0.040896, 0.037267, 0.038286, 0.43299, 0.2515, 0.89362, 0.23729, 0.30657, 80, 1.5E-4, 4, 1.0E-2, 2.6E-5, 1.0, 0.1, 0.1, 0.1, 1.23E-3, 0.1, 3000, 40, 1E-6 ];
end
...
if ( ~suppress_plot )
    ...
    title('Swap_4_10 observables','fontSize',14,'Interpreter','none');
...
function [val] = rateLaw__7(expressions, observables)
    val = (expressions(23)*RBS4Removal(expressions,observables));
end
...
function [val] = rateLaw__17(expressions, observables)
    val = (expressions(29)*RBS10Removal(expressions,observables));
end
...
function [ ratelaws ] = calc_ratelaws ( species, expressions, observables )
    ...
    ratelaws(29) = rateLaw__7(expressions,observables)*species(26)*species(42);
    ...
    ratelaws(39) = rateLaw__17(expressions,observables)*species(32)*species(36);
...
function [ Dspecies ] = calc_species_deriv ( time, species, expressions )
    ...
    Dspecies(36) = ratelaws(27) -ratelaws(39) -ratelaws(45) +ratelaws(46);
    ...
    Dspecies(42) = ratelaws(22) -ratelaws(29) -ratelaws(69) +ratelaws(70);
