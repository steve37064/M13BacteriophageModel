function [err, timepoints, species_out, observables_out] = Swap_8_9( timepoints, species_init, parameters, suppress_plot )
%SWAP_8_9 Integrate reaction network and plot observables.
%   Integrates the reaction network corresponding to the BioNetGen model
%   'Swap_8_9' and then (optionally) plots the observable trajectories,
...
%   'ode15s'. SWAP_8_9 returns an error value, a vector of timepoints,
%   species trajectories, and observable trajectories.
%   
%   [err, timepoints, species_out, observables_out]
%        = Swap_8_9( timepoints, species_init, parameters, suppress_plot )
%
%   INPUTS:
%   -------
...
if ( ~suppress_plot )
    ...
    title('Swap_8_9 observables','fontSize',14,'Interpreter','none');
...
function [val] = rateLaw__15(expressions, observables)
    val = (((0.3*expressions(27))*RBS8Removal(expressions,observables))*RBS9Removal(expressions,observables));
end

% function rateLaw__16
function [val] = rateLaw__16(expressions, observables)
    val = ((expressions(28)*RBS8Removal(expressions,observables))*RBS9Removal(expressions,observables));
end
...
function [ ratelaws ] = calc_ratelaws ( species, expressions, observables )
    ...
    ratelaws(37) = rateLaw__15(expressions,observables)*species(30)*species(39)*species(38);
    ratelaws(38) = rateLaw__16(expressions,observables)*species(31)*species(39)*species(38);
