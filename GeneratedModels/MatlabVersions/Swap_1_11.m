function [err, timepoints, species_out, observables_out] = Swap_1_11( timepoints, species_init, parameters, suppress_plot )
%SWAP_1_11 Integrate reaction network and plot observables.
%   Integrates the reaction network corresponding to the BioNetGen model
%   'Swap_1_11' and then (optionally) plots the observable trajectories,
...
%   'ode15s'. SWAP_1_11 returns an error value, a vector of timepoints,
%   species trajectories, and observable trajectories.
%   
%   [err, timepoints, species_out, observables_out]
%        = Swap_1_11( timepoints, species_init, parameters, suppress_plot )
%
%   INPUTS:
%   -------
...
if ( ~suppress_plot )
    ...
    title('Swap_1_11 observables','fontSize',14,'Interpreter','none');
...
function [val] = rateLaw__18(expressions, observables)
    val = ((((expressions(30)*RBS3Removal(expressions,observables))*RBS6Removal(expressions,observables))*RBS11Removal(expressions,observables))*RBS1Removal(expressions,observables));
end
...
function [ ratelaws ] = calc_ratelaws ( species, expressions, observables )
    ...
    ratelaws(40) = rateLaw__18(expressions,observables)*species(33)*species(41)*species(43)*species(44)*species(40);
