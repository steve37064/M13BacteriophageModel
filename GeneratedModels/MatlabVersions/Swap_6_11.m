function [err, timepoints, species_out, observables_out] = Swap_6_11( timepoints, species_init, parameters, suppress_plot )
%SWAP_6_11 Integrate reaction network and plot observables.
%   Integrates the reaction network corresponding to the BioNetGen model
%   'Swap_6_11' and then (optionally) plots the observable trajectories,
...
%   'ode15s'. SWAP_6_11 returns an error value, a vector of timepoints,
%   species trajectories, and observable trajectories.
%   
%   [err, timepoints, species_out, observables_out]
%        = Swap_6_11( timepoints, species_init, parameters, suppress_plot )
%
%   INPUTS:
%   -------
...
if ( isempty(parameters) )
   parameters = [ 0.1, 0.125, 1.0, 1.0E3, 1, 9.5454e-16, 1E-3, 1100, 1.0, 6E-3, 1E-2, 6.5E-3, 2E-4, 2.25E-3, 1.34, 0.03643, 0.06775, 0.25, 0.05492, 0.04295, 0.60, 0.02377, 0.00557, 0.00307, 0.00307, 0.00307, 0.00163, 0.00163, 0.00163, 0.00163, 0.00163, 1.60, 10.8, 8.00, 4.60, 20.0, 0.80, 4.40, 1.60, 1.08, 4.00, 0.25, 0.048443, 0.040896, 0.037267, 0.038286, 0.43299, 0.2515, 0.89362, 0.23729, 0.30657, 80, 1.5E-4, 4, 1.0E-2, 2.6E-5, 1.0, 0.1, 0.1, 0.1, 1.23E-3, 0.1, 3000, 40, 1E-6 ];
end
...
if ( ~suppress_plot )
    ...
    title('Swap_6_11 observables','fontSize',14,'Interpreter','none');
...
function [val] = rateLaw__18(expressions, observables)
    val = ((((expressions(30)*RBS3Removal(expressions,observables))*RBS11Removal(expressions,observables))*RBS1Removal(expressions,observables))*RBS6Removal(expressions,observables));
end

% function rateLaw__19
function [val] = rateLaw__19(expressions, observables)
    val = ((expressions(31)*RBS3Removal(expressions,observables))*RBS11Removal(expressions,observables));
end
...
function [ ratelaws ] = calc_ratelaws ( species, expressions, observables )
    ...
    ratelaws(40) = rateLaw__18(expressions,observables)*species(33)*species(41)*species(44)*species(40)*species(43);
    ratelaws(41) = rateLaw__19(expressions,observables)*species(34)*species(41)*species(44);
...
function [ Dspecies ] = calc_species_deriv ( time, species, expressions )
    ...
    Dspecies(43) = ratelaws(26) -ratelaws(40) -ratelaws(60) +ratelaws(61);
