function [err, timepoints, species_out, observables_out] = Swap_3_6( timepoints, species_init, parameters, suppress_plot )
%SWAP_3_6 Integrate reaction network and plot observables.
%   Integrates the reaction network corresponding to the BioNetGen model
%   'Swap_3_6' and then (optionally) plots the observable trajectories,
...
%   'ode15s'. SWAP_3_6 returns an error value, a vector of timepoints,
%   species trajectories, and observable trajectories.
%   
%   [err, timepoints, species_out, observables_out]
%        = Swap_3_6( timepoints, species_init, parameters, suppress_plot )
%
%   INPUTS:
%   -------
...
if ( ~suppress_plot )
    ...
    title('Swap_3_6 observables','fontSize',14,'Interpreter','none');
...
function [val] = rateLaw__18(expressions, observables)
    val = ((((expressions(30)*RBS6Removal(expressions,observables))*RBS3Removal(expressions,observables))*RBS1Removal(expressions,observables))*RBS11Removal(expressions,observables));
end

% function rateLaw__19
function [val] = rateLaw__19(expressions, observables)
    val = ((expressions(31)*RBS6Removal(expressions,observables))*RBS3Removal(expressions,observables));
end
...
function [ ratelaws ] = calc_ratelaws ( species, expressions, observables )
    ...
    ratelaws(40) = rateLaw__18(expressions,observables)*species(33)*species(43)*species(41)*species(40)*species(44);
