function [err, timepoints, species_out, observables_out] = Swap_4_5( timepoints, species_init, parameters, suppress_plot )
%SWAP_4_5 Integrate reaction network and plot observables.
%   Integrates the reaction network corresponding to the BioNetGen model
%   'Swap_4_5' and then (optionally) plots the observable trajectories,
...
%   'ode15s'. SWAP_4_5 returns an error value, a vector of timepoints,
%   species trajectories, and observable trajectories.
%   
%   [err, timepoints, species_out, observables_out]
%        = Swap_4_5( timepoints, species_init, parameters, suppress_plot )
%
%   INPUTS:
%   -------
...
if ( isempty(parameters) )
   parameters = [ 0.1, 0.125, 1.0, 1.0E3, 1, 9.5454e-16, 1E-3, 1100, 1.0, 6E-3, 1E-2, 6.5E-3, 2E-4, 2.25E-3, 1.34, 0.0236, 0.03369, 0.25, 0.30455, 0.04214, 0.60, 0.13099, 0.0307, 0.01689, 0.01689, 0.01689, 0.00163, 0.00163, 0.00163, 0.00163, 0.00163, 1.60, 10.8, 8.00, 4.60, 20.0, 0.80, 4.40, 1.60, 1.08, 4.00, 0.25, 0.048443, 0.040896, 0.037267, 0.038286, 0.43299, 0.2515, 0.89362, 0.23729, 0.30657, 80, 1.5E-4, 4, 1.0E-2, 2.6E-5, 1.0, 0.1, 0.1, 0.1, 1.23E-3, 0.1, 3000, 40, 1E-6 ];
end
...
if ( ~suppress_plot )
    ...
    title('Swap_4_5 observables','fontSize',14,'Interpreter','none');
...
function [val] = rateLaw__9(expressions, observables)
    val = ((((0.3*expressions(24))*RBS4Removal(expressions,observables))*RBS9Removal(expressions,observables))*RBS8Removal(expressions,observables));
end

% function rateLaw__11
function [val] = rateLaw__11(expressions, observables)
    val = ((((0.3*expressions(25))*RBS4Removal(expressions,observables))*RBS9Removal(expressions,observables))*RBS8Removal(expressions,observables));
end

% function rateLaw__12
function [val] = rateLaw__12(expressions, observables)
    val = ((0.7*expressions(26))*RBS4Removal(expressions,observables));
end

% function rateLaw__13
function [val] = rateLaw__13(expressions, observables)
    val = ((((0.3*expressions(26))*RBS4Removal(expressions,observables))*RBS9Removal(expressions,observables))*RBS8Removal(expressions,observables));
end
...
function [val] = rateLaw__17(expressions, observables)
    val = (expressions(29)*RBS5Removal(expressions,observables));
end
...
function [ ratelaws ] = calc_ratelaws ( species, expressions, observables )
    ...
    ratelaws(31) = rateLaw__9(expressions,observables)*species(27)*species(42)*species(38)*species(39);
    ratelaws(32) = (0.7*expressions(25))*species(28);
    ratelaws(33) = rateLaw__11(expressions,observables)*species(28)*species(42)*species(38)*species(39);
    ratelaws(34) = rateLaw__12(expressions,observables)*species(29)*species(42);
    ratelaws(35) = rateLaw__13(expressions,observables)*species(29)*species(42)*species(38)*species(39);
    ...
    ratelaws(39) = rateLaw__17(expressions,observables)*species(32)*species(37);
...
function [ Dspecies ] = calc_species_deriv ( time, species, expressions )
    ...
    Dspecies(37) = ratelaws(27) -ratelaws(39) -ratelaws(48) +ratelaws(49);
    ...
    Dspecies(42) = ratelaws(21) +ratelaws(22) -ratelaws(31) -ratelaws(33) -ratelaws(34) -ratelaws(35) -ratelaws(69) +ratelaws(70);
